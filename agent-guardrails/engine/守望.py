...
# ⭐⭐ 夜-1（2026-08-18）：在岗心跳。
#   ⚠️ 病灶：本件原来**不写任何在岗凭据** ⇒ 布防了和没布防**从外面长得一模一样**。
#     它有布防声/撤防声/空守望警告，但那些只**打给屏幕** ——
#     事后、别的窗、任何脚本都无从知道它此刻在不在岗。
#   ⇒ 于是「一夜没告警」有两种含义（真平安 / 根本没人在盯），
#     ⭐ 而那正是本件在文件头反复要消灭的东西 —— 它自己犯了。
#   ⭐ 形状对齐 `卡住哨.py` 的 写心跳()，⛔ 别另造一套。
def _心跳路径():
    """⭐ 与 `夜班.py` / `卡住哨.py` 同一套解析：环境变量优先，否则 <引擎的上一级>/_state。
    ⛔ 两边必须一致——不一致就会变成「它在写、别人在别处找」，又是一次假的判不了。"""
    import os
    根 = Path(__file__).resolve().parent.parent
    d = Path(os.environ.get("WORLDBOOK_STATE_DIR") or (根 / "_state"))
    return d / "守望心跳.json"


心跳最短间隔 = 5.0   # ⚠️ 拍的：远小于读方（夜班.py）的 300 秒陈旧阈值 ⇒ ⛔ 不会被误判掉线；
                     #   又远大于最快的 --隔 0.05 ⇒ ⛔ 不再每轮写盘。⛔ 无实测依据，撞上再校。
_上次心跳 = [0.0]


def 写心跳(状态="在岗", 轮=0, 盯几个=0, 谁="", 必写=False):
    """⛔ 写不进去**不许拖垮守望本身**——最坏退回"看不见在岗"，同卡住哨的口径。

    ⚠️⚠️ **必须节流**（2026-08-18 被回归打红才发现）：
      我第一版把它放进每一轮循环，而 `--隔` 可以低到 0.05 秒
      ⇒ **每秒 20 次原子写盘** ⇒ 循环被拖慢、时序漂掉，把「冷却内的改动要补报」那条打红了。
      ⭐ 那条红是**对的** —— 心跳根本不需要那么勤（读方用 300 秒阈值）。
    ⭐ `必写=True` 用于**布防那一次和两处撤防**：它们各只发生一次，
      且正是「在岗／主动撤了／被杀」分得开的关键 ⇒ ⛔ 不许被节流吃掉。
    """
    import time as _tm
    if not 必写 and (_tm.time() - _上次心跳[0]) < 心跳最短间隔:
        return
    _上次心跳[0] = _tm.time()
    try:
        ...
        tmp.write_text(_json.dumps({"时刻": time.time(), "状态": 状态, "轮": 轮,
                                    "盯": 盯几个, "谁": 谁, "pid": _os.getpid()},
...
def 主():
    ...
    # ⭐⭐ 2026-08-18：`--除` 按**路径**排除，治不了**共享目录**（如收件箱）——
    #   我写的信和别人写的信落在同一路径下 ⇒ 路径上分不开 ⇒ **自己投信自己醒**（实测一晚 3 次）。
    #   ⚠️ 那不是无害噪音：总在你自己动作上叫的哨兵会训练你忽略它，
    #     而它一旦被忽略，**真正该看的那一声也会被一起忽略**。
    #   ⇒ 真正的判据在**内容**里（信的抬头写着 `从: …`）。
    ap.add_argument("--除内容", nargs="*", default=[],
                    help="⭐ 按**内容**排除：变化文件头部含这些串之一 ⇒ ⛔ 不报。"
                         "治共享目录里「自己动自己醒」（如 `--除内容 \"从: xx窗\"`）。"
                         "⚠️ 读不出来时**照报** —— ⛔ 宁可多一声，也不许静默吞掉一次变化")
    a = ap.parse_args()
    ...
    内容尾 = f" · 按内容排除 {len(a.除内容)} 条" if a.除内容 else ""
    print(f"🛡️ 守望已布防 {谁}· 盯 {len(基线)} 个文件（{len(a.盯)} 个目标，排除 {len(a.除)} 处）"
          f"{内容尾}{读失败尾}"
    ...
    # ⭐ 布防那一刻就写心跳，⛔ 别等第一轮 —— 否则前 --隔 秒是"没人在岗"（同卡住哨的注释）
    写心跳("在岗", 0, len(基线), a.谁, 必写=True)
    ...
    try:
        while True:
            ...
            待报.clear()
            # ⭐ 内容排除：只在**要报的这一刻**读，且只读头部 ⇒ ⛔ 不影响轮询开销。
            #   ⚠️ 读不出来 ⇒ **照报**（⛔ 不许因为读不了就静悄悄吞掉一次变化）。
            if a.除内容:
                留 = []
                for t, q in 变:
                    try:
                        头 = Path(q).read_text(encoding="utf-8", errors="replace")[:800]
                    except Exception:
                        留.append((t, q))          # ⛔ 读不了 ⇒ 照报
                        continue
                    if not any(k and k in 头 for k in a.除内容):
                        留.append((t, q))
                变 = 留
                if not 变:
                    # ⚠️ 全被排掉 ⇒ **这一声不发**；⭐ 且把冷却还原
                    #   （这次没真报，⛔ 别让它白占一次冷却额度）。
                    #   ⛔ 也不打印"跳过了几条"——打印了就等于没排除。
                    上次报 = 0.0
                    continue
            名 = " ".join(f"{短名(p)}({t})" for t, p in 变[:a.最多列])
            余 = f" 等共 {len(变)} 处" if len(变) > a.最多列 else ""
            读失败尾 = f" · ⚠️ 另有 {读不了} 处读不了" if 读不了 else ""
            报过 += 1      # ⭐ 移到这里：只有**真的要报**才 +1，⛔ 别把被内容排掉的也算进上限
            print(f"📣 守望｜{谁}别人动了你盯的：{名}{余}{读失败尾} ｜ {醒来清单}", flush=True)
            if 报过 >= a.上限:
                # ② 撤防声（触顶）。⛔ 不自行续命——一次布防 ＝ 一次预算。
                # ⭐ 撤防也要写进心跳 ⇒ 「主动撤了」和「被杀/崩了」从外面分得开
                写心跳("已撤防·报满上限", 报过, len(基线), a.谁, 必写=True)
                print(f"🛡️ 守望撤防：已报满 {a.上限} 次上限 ⇒ ⛔ 不自行续命。"
                      f"还要盯就重新布防（⭐ 先想清楚为什么这么吵）", flush=True)
                return 0
    except KeyboardInterrupt:
        写心跳("已撤防·被叫停", 报过, len(基线), a.谁, 必写=True)
